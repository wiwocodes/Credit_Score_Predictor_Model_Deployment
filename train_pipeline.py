...
Pipeline training berbasis OOP dengan MLflow tracking.
Struktur:
  - DataLoader       : memuat dan melakukan initial cleaning data
  - Preprocessor     : imputation, encoding, scaling (fit on train only)
  - ModelTrainer     : training setiap model ML
  - ModelEvaluator   : evaluasi metrik dan logging ke MLflow
  - Pipeline         : orkestrasi seluruh proses end-to-end
"""
...
class Preprocessor:
    ...
    Menangani:
      ...
      - IQR-based median imputation (fit on train only)
      - Label Encoding untuk fitur kategorikal
      - StandardScaler untuk fitur numerik
    Semua transformer di-fit HANYA pada training set untuk menghindari data leakage.
    """
...
class ModelTrainer:
    """
    Menyimpan definisi semua model dan melakukan training.
    Setiap model dapat di-train dan di-retrieve secara individual.
    """
...
class ModelEvaluator:
    """
    Evaluasi setiap model dan log semua hasil ke MLflow.
    Menghasilkan:
      - Accuracy & F1-Score (Macro)
      - Classification report (teks)
      - Confusion matrix (gambar artifact)
      - Model comparison chart (gambar artifact)
      - Feature importance chart (jika tersedia)
    """
...
class Pipeline:
    ...
    Orkestrasi end-to-end:
      ...
      3. Train semua model
      4. Evaluate & log ke MLflow (satu run per model)
      5. Log model terbaik sebagai registered model
    """
